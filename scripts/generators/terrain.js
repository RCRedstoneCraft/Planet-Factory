...
function generateMap (seed) {
    ...
    for (let i = 0; i < map.starSystems.length; i++) {
        map.starSystems[i] = generateStarSystem(seed, i);
    }
    console.log("Map generation done");
    return map;
}

function generateStarSystem(seed, starId) {
    console.log("\tGenerating Star System (ID: " + starId + ")");

    let rng = seededRandom(seed * starCount + starId);
    let starSystem = {
    ...
    for (let i = 0; i < starSystem.planets.length; i++) {
        starSystem.planets[i] = generatePlanet(seed, starId, i);
    }
    return starSystem;
}

function generatePlanet(seed, starId, planetId) {
    console.log("\t\tGenerating Planet (ID: " + planetId + ")");
    let rngSeed = (seed * starCount + starId) * planetCount + planetId
    let rng = seededRandom(rngSeed);
    let planet = {
        heightMap: heightMap = generateWhiteNoise(planetSize, rngSeed),                                                                   // TODO
        oreMap: null,                                                                                                             // TODO
        starDistance: 0,
